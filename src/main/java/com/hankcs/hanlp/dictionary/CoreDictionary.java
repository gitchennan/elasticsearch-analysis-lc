...
//        if (loadDat(path)) return true;
        ...
        try {
            ...
            HanLpLogger.info(CoreDictionary.class, "核心词典读入词条" + map.size() + " 全部频次" + MAX_FREQUENCY + "，耗时" + (System.currentTimeMillis() - start) + "ms");
            br.close();
            int buildTrieResult = trie.build(map);
            HanLpLogger.info(CoreDictionary.class, "[coreDictionary] build trie result=" + buildTrieResult);
