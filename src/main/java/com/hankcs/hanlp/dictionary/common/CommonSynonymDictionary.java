...
public class CommonSynonymDictionary {
    ...
    public boolean load(InputStream inputStream) {
        ...
        try {
            ...
            if (resultCode != 0) {
                HanLpLogger.warn(CommonSynonymDictionary.class, "构建" + inputStream + "失败，错误码" + resultCode);
                return false;
            }
        }
        catch (Exception e) {
            HanLpLogger.error(CommonSynonymDictionary.class, "读取" + inputStream + "失败，可能由行" + line + "造成");
            return false;
        }
        return true;
    }
