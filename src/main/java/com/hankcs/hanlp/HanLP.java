...
import com.hankcs.hanlp.utility.IOSafeHelper;
...
public class HanLP {
    ...
    public static final class Config {
        ...
        /**
         * IO适配器（默认null，表示从本地文件系统读取），实现com.com.hankcs.hanlp.corpus.io.IIOAdapter接口
         * 以在不同的平台（Hadoop、Redis等）上运行HanLP
         */
        public static IIOAdapter IOAdapter;
//
//        public static IIOAdapter IOAdapter = new IIOAdapter() {
//            @Override
//            public InputStream open(String path) throws IOException {
//                File inputFile = new File(path);
//                if (!inputFile.exists()) {
//                    inputFile = getPluginDirPath().toAbsolutePath().resolve(path).toFile();
//                    if (!inputFile.exists()) {
//                        logger.log(Level.WARNING, String.format("[IIOAdapter] open(), can't find file[%s]", inputFile.getAbsolutePath()));
//                        return null;
//                    }
//                }
//                return IOSafeHelper.openFileInputStream(inputFile.getAbsolutePath());
//            }
//
//            @Override
...
//                    outputFile = getPluginDirPath().toAbsolutePath().resolve(path).toFile();
//                    if (!outputFile.exists()) {
//                        logger.log(Level.WARNING, String.format("[IIOAdapter] create(), can't find file[%s]", outputFile.getAbsolutePath()));
//                    }
//                }
//                return IOSafeHelper.openFileOutputStream(outputFile.getAbsolutePath());
//            }
//
//            public Path getPluginDirPath() {
//                return PathUtils
//                        .get(new File(AnalysisHanLPPlugin.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getParent())
//                        .toAbsolutePath();
//            }
//        };
            ...
            try {
                ...
                try {
                    reader = new InputStreamReader(
                            Predefine.HANLP_PROPERTIES_PATH == null ?
                                    loader.getResourceAsStream("hanlp.properties") : new FileInputStream(Predefine.HANLP_PROPERTIES_PATH), "UTF-8");
                    p.load(reader);
                }
                catch (Exception ex) {
                    IOSafeHelper.closeReader(reader);
                    throw ex;
                }
