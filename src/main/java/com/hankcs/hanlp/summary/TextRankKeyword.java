...
public class TextRankKeyword extends KeywordExtractor {
    ...
    public List<String> getKeyword(String content) {
        ...
        List<String> result = Lists.newArrayListWithCapacity(entrySet.size());
    ...
    public Map<String, Float> getRank(List<Term> termList) {
        List<String> wordList = Lists.newArrayListWithCapacity(termList.size());
        for (Term t : termList) {
            if (shouldInclude(t)) {
                wordList.add(t.word);
            }
        ...
        Map<String, Set<String>> words = Maps.newTreeMap();
        Queue<String> que = Lists.newLinkedList();
        for (String w : wordList) {
            if (!words.containsKey(w)) {
                words.put(w, Sets.newTreeSet());
            }
            que.offer(w);
            if (que.size() > 5) {
                que.poll();
