...
public class Occurrence {
    ...
    public double computeMutualInformation(PairFrequency pair) {
        return Math.log(Math.max(Predefine.MIN_PROBABILITY, pair.getValue() / totalPair) / Math.max(Predefine.MIN_PROBABILITY, (CoreDictionary.INSTANCE.getTermFrequency(pair.first) / (double) CoreDictionary.TOTAL_FREQUENCY * CoreDictionary.INSTANCE.getTermFrequency(pair.second) / (double) CoreDictionary.TOTAL_FREQUENCY)));
    }
